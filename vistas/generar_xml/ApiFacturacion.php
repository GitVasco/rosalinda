...
{
	public function EnviarComprobanteElectronico($emisor, $nombre, $rutacertificado = "vistas/generar_xml/", $ruta_archivo_xml = "vistas/generar_xml/archivos_xml/", $ruta_archivo_cdr = "vistas/generar_xml/cdr/")
	{
		...
		//*nuevos guia
		//beta
		//$ws = 'https://e-beta.sunat.gob.pe/ol-ti-itemision-guia-gem-beta/billService?wsdl';
		//produccion
		//$ws = 'https://e-guiaremision.sunat.gob.pe/ol-ti-itemision-guia-gem/billService?wsdl';
		...
		if ($httpcode == 200) //200: La comunicacion fue satisfactoria
		{
			...
			if (isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)) // si en la etique de rpta hay valor entra
			{
				...
				echo '<script>
                            ...
                            swal({
                                    type: "success",
                                    title: "Se envio el archivo XML correctamente",
				...
				echo '<script>
                    swal({
                        type: "warning",
                        title: "Alerta",
                        text: "¡Ocurrio un error al enviar el archivo a la SUNAT!",
                        showConfirmButton: true,
                        confirmButtonText: "Cerrar"
                    }).then((result)=>{
                        if(result.value){
	...
	public function EnviarResumenComprobantes($emisor, $nombre, $rutacertificado = "vistas/generar_xml/", $ruta_archivo_xml = "vistas/generar_xml/archivos_xml/")
	{
		//firma del documento
		...
		$ruta_firma = $rutacertificado . "10472810371.pfx";
	...
	function ConsultarTicket($emisor, $cabecera, $ticket, $ruta_archivo_cdr = "vistas/generar_xml/cdr/")
	{
		$ws = "https://e-beta.sunat.gob.pe/ol-ti-itcpfegem-beta/billService";
