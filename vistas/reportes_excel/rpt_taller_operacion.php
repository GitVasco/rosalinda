...
# Titulo de la hoja
$objPHPExcel->getActiveSheet()->setTitle("TALLERES OPERACION SERVICIO -".$fecha);
...
// TITULO
$fila = 2;
$objPHPExcel->getActiveSheet()->SetCellValue("D$fila", 'TALLER GENERADO');
...
$respDetalle = ControladorTalleres::ctrRangoFechasTalleresOperaciones($modelo);
$cont = 0;
for ($i=0; $i < count ($respDetalle); $i++) { 
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue("B$fila", utf8_encode($respDetalle[$i]["codigo"])); 
    
    $objPHPExcel->getActiveSheet()->SetCellValue("C$fila", utf8_encode($respDetalle[$i]["modelo"]));
    
    $objPHPExcel->getActiveSheet()->SetCellValue("D$fila", utf8_encode($respDetalle[$i]["color"]));
    
    $objPHPExcel->getActiveSheet()->SetCellValue("E$fila", utf8_encode($respDetalle[$i]["talla"]));

    $objPHPExcel->getActiveSheet()->SetCellValue("F$fila", utf8_encode($respDetalle[$i]["nom_operacion"]));


    $objPHPExcel->getActiveSheet()->SetCellValue("G$fila", utf8_encode($respDetalle[$i]["cantidad"]));

    $objPHPExcel->getActiveSheet()->SetCellValue("H$fila", utf8_encode($respDetalle[$i]["fecha"]));

    if($respDetalle[$i]["estado"] == 1){
    $objPHPExcel->getActiveSheet()->SetCellValue("I$fila", utf8_encode("Generado"));
    }else if($respDetalle[$i]["estado"] == 2){
        $objPHPExcel->getActiveSheet()->SetCellValue("I$fila", utf8_encode("En Proceso"));
    }else{
        $objPHPExcel->getActiveSheet()->SetCellValue("I$fila", utf8_encode("Terminado"));
    }
...
# Nombre del archivo
header('Content-Disposition: attachment; filename=" TALLERES OPERACIONES SERVICIO - '.$fecha.'.xls"');
